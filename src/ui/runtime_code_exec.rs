...
pub(crate) fn handle_code_exec_exit(
    ...
    let Some((pending, content)) = take_pending_and_output(&mut tab_state.app) else {
        return;
    };
...
pub(crate) fn handle_code_exec_approve(
    ...
    let Some(mut pending) = clone_pending_or_notify(&mut tab_state.app) else {
        return;
    };
...
pub(crate) fn handle_code_exec_deny(
    ...
    let Some(pending) = take_pending_or_notify(&mut tab_state.app) else {
        return;
    };
