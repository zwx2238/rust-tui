...
impl Widget for JumpWidget {
    ...
    fn render(&mut self, frame: &mut WidgetFrame<'_, '_>) -> Result<(), Box<dyn Error>> {
        ...
        frame.jump_rows.extend(crate::ui::overlay_render::build_jump_overlay_rows(
            frame.view,
            frame.ctx,
        ));
        clamp_overlay_tables(frame.view, frame.ctx, frame.jump_rows.len());
        crate::ui::overlay_render::render_jump_overlay(frame.ctx, frame.view, frame.jump_rows)?;
        Ok(())
    }
}
