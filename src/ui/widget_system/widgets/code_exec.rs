...
use crate::ui::runtime_dispatch::mouse_overlay::handle_code_exec_overlay_mouse;
...
fn update_overlay_state(ctx: &mut UpdateCtx<'_>) {
    if ctx.view.overlay.is(crate::ui::overlay::OverlayKind::CodeExec) {
        let should_close = bind_active_tab(ctx.tabs, *ctx.active_tab)
            .map(|mut binding| binding.app().pending_code_exec.is_none())
            .unwrap_or(true);
        if should_close {
...
fn overlay_event(
    ...
    match event {
        ...
        crossterm::event::Event::Mouse(m) => {
            if handle_code_exec_overlay_mouse(m, &mut binding.dispatch, binding.layout, binding.view) {
                return Ok(false);
            }
