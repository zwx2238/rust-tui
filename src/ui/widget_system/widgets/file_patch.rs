...
use crate::ui::runtime_dispatch::mouse_overlay::handle_file_patch_overlay_mouse;
...
impl Widget for FilePatchWidget {
    ...
    fn render(&mut self, frame: &mut WidgetFrame<'_, '_>) -> Result<(), Box<dyn Error>> {
        crate::ui::overlay_render_tool::render_file_patch_overlay(frame.ctx)
    }
}

fn update_overlay_state(ctx: &mut UpdateCtx<'_>) {
    if ctx.view.overlay.is(crate::ui::overlay::OverlayKind::FilePatch) {
        let should_close = bind_active_tab(ctx.tabs, *ctx.active_tab)
            .map(|mut binding| binding.app().pending_file_patch.is_none())
            .unwrap_or(true);
        if should_close {
...
fn overlay_event(
    ...
    match event {
        ...
        crossterm::event::Event::Mouse(m) => {
            if handle_file_patch_overlay_mouse(m, &mut binding.dispatch, binding.layout, binding.view) {
                return Ok(false);
            }
