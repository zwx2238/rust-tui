...
pub(crate) fn draw_notice(f: &mut Frame<'_>, area: Rect, app: &mut App, theme: &RenderTheme) {
    let Some(notice) = active_notice(app) else { return; };
    if !has_space(area) { return; }
...
fn active_notice(app: &mut App) -> Option<Notice> {
    ...
    if Instant::now() >= notice.expires_at { app.notice = None; return None; }
    Some(notice.clone())
}
...
fn build_popup_config(area: Rect, text: &str, theme: &RenderTheme) -> PopupConfig {
    ...
    config.position = PopupPosition::Absolute { x, y, width: popup_width, height: popup_height };
...
fn measure_popup(area: Rect, text: &str) -> (u16, u16) {
    ...
    let content_width = wrapped.iter().map(|l| UnicodeWidthStr::width(l.as_ref())).max().unwrap_or(0);
