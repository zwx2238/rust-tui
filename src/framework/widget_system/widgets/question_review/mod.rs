...
pub(crate) use popup::{question_review_list_area, question_review_popup_area};
...
fn table_title(pending: &PendingQuestionReview) -> String {
    ...
    format!(
        "批量提问 · 未确认 {pending_count} · 通过 {approved} · 拒绝 {rejected} · A 全通过 · m/M 切模型 · Alt+M 全同模型 · PgUp/PgDn 滚动详情 · Enter 提交 · Esc 取消"
    )
}
